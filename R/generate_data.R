...
mean_vec <- scale_between(master_df$Age, 0.7, 1.2) * 
  scale_between(master_df$Genetic_disposition, 0.7, 1.3) * 
  scale_between(max(master_df$Packs_per_day) - master_df$Packs_per_day, 1, 1.8) *
  scale_between(max(master_df$Exercise_per_week) - master_df$Exercise_per_week, 0.8, 1.2)
mean_vec <- scale_between(mean_vec, 80, 120)
master_df$Cholesterol_HDL <- rnorm(n, mean_vec, 15)
mean_vec <- scale_between(master_df$Cholesterol_HDL, 35, 45)
